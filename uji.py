...
class ngentod:
    ...
    def romiy(self):
        ...
        if unikers in ('Y', 'y'):
            print '\n %s[%s!%s] contoh : %ssayang,anjing,kontol%s gunakan , (koma) untuk pemisah '%(P,M,P,H,P)
            while True:
		...
		try:
			...
			with requests.Session() as ses:
				...
				if "checkpoint_url" in str(data):
					...
					print ("\r%s[!] Aktifkan pesawatkan 2 detik!!  "%(M)),
					looping+=1
					sys.stdout.flush()
					pwx = [pw]
					crack2(user, pwx)
			...
			print ("\r%s[!] Tidak ada koneksi Internet kentod!! "%(M)),
			sys.stdout.flush()
			looping+=1
			pwx = [pw]
			crack2(user, pwx)
...
def menu():
    ...
    print (' [%s05%s] %sStart crack/Mulai crack %s'%(K,P,H,P)) 
